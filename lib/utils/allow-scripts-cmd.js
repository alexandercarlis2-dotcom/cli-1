...
class AllowScriptsCmd extends BaseCommand {
  ...
  findNodesForArgs (args, arb) {
    ...
    // use the URL-derived name; non-registry deps use their installed name.
    // A version or range on the arg narrows the match to installed
    ...
    for (const arg of args) {
      ...
      for (const node of arb.actualTree.inventory.values()) {
        ...
        const { name, version } = node.isRegistryDependency === false
          ? node
          : trustedDisplay(node)
