...
// installed versions.
//
// Denying writes the widest trusted identity available: a name-only registry
// key, a hosted-git repository without a commit, or the exact resolved source
// for file and remote dependencies. `--allow-scripts-pin` does not affect denies.

const primaryResolvedSource = (node) => resolvedSourceSpecs(node)[0] || ''
const hasIncomingRemoteEdge = (node) => {
  if (!node?.edgesIn || typeof node.edgesIn[Symbol.iterator] !== 'function') {
    return false
  }
  for (const edge of node.edgesIn) {
    if (/^https?:\/\//.test(edge?.spec || '')) {
      return true
    }
  }
  return false
}
// Registry packages also resolve to HTTP tarballs, so URL shape alone cannot
// identify a direct remote dependency. Arborist derives this flag from the
// incoming dependency edges, which are independent of the tarball manifest.
const isDirectRemote = (node, resolved) =>
  node.isRegistryDependency === false &&
  /^https?:\/\//.test(resolved) &&
  hasIncomingRemoteEdge(node)
...
// for direct remote installs, hosted shortcut for git, the resolved file path
// for local installs). Never falls back to `node.packageName` / `node.version`,
...
const versionedKeyFor = (node) => {
  ...
  if (isDirectRemote(node, resolved)) {
    return resolved
  }
...
// `npm install-scripts deny <name>`; name-only registry, git, file, and
// remote denies must be removed to approve the current install.
