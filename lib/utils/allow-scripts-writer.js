...
const hasIncomingSpec = (node, matches) => {
  for (const edge of incomingEdges(node)) {
    if (matches(edge?.spec)) {
      return true
    }
  }
  return false
...
const exactFileKeyFor = (node, resolved) => {
  if (!node?.resolved && node?.linksIn &&
    ...
    for (const link of node.linksIn) {
      ...
      const target = parseSpec(node.realpath || node.path)
      return target?.type === 'file' || target?.type === 'directory'
        ? target.saveSpec
        : null
    }
  }
...
const versionedKeyFor = (node) => {
  ...
  const fileKey = exactFileKeyFor(node, resolved)
