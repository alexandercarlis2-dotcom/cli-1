...
For a file tarball, approval writes the resolved file spec. For a linked
directory, it writes the canonical physical target path so identical relative
link strings cannot authorize different directories. File identities stay
exact with `--no-allow-scripts-pin`.
